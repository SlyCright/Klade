...
class SimulationIntegrationTest {
    ...
    void givenSimulationWithDefaultSettings_whenRunningMultipleGenerations_thenAverageFitnessImprovesOverTime() {
        ...
        for (int generation = 0; generation < generationsToRun; generation++) {
            ...
            int speciesCount = 0;
            int specimenCount = 0;
            for (Species species : simulation.getGeneration().getSpeciesList()) {
                for (Genome genome : species.getGenomes()) {
                    totalFitness += genome.getAccumulatedFitness();
                    specimenCount++;
                    System.out.printf("Specimen %d of species %d, fitness: %.3f%n", specimenCount, speciesCount, genome.getAccumulatedFitness());
                }
                speciesCount++;
            }
